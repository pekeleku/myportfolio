...
export default function SkillCard({
  name,
  icon,
  gradientColor = "from-red-600 to-red-900",
}) {
  return (
    <div className="skill-card group cursor-pointer ">
      ...
      <div className="relative mb-4">
        <div
          className={`absolute inset-0 bg-gradient-to-r ${gradientColor} rounded-2xl blur-xl opacity-0 group-hover:opacity-40 transition-opacity duration-500`}
        ></div>
      ...
      {/* Glow line */}
      <div
        className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r ${gradientColor} group-hover:w-3/4 transition-all duration-300 rounded-full`}
      ></div>
    </div>
  );
}
